...
module.exports = {
  ...
  apply: async (
    env,
    options = { async startTask() {}, async endTask() {} }
  ) => {
    ...
    if (checkDeployConfigExists()) {
      ...
      if (env) {
        ...
        if (answers.confirm) {
          startTask && (await startTask([cluster]))
